...
tree, root metadata, fixed spacing widths, fixed table-cell alignment values,
and fixed `mathvariant="normal"` values on identifiers match the typesetter's
strict allowlist. Relative links and images are resolved against the
