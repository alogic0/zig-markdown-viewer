...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len != 3) return fatal(init, "usage: strip-wasm-metadata INPUT OUTPUT", .{});
    ...
    const output = stripMetadataSections(init.gpa, input) catch |err| {
...
fn stripMetadataSections(allocator: std.mem.Allocator, input: []const u8) (Error || std.Io.Writer.Error || std.mem.Allocator.Error)![]u8 {
    ...
    while (cursor < input.len) {
        ...
        const remove = input[section_start] == 0 and try isBuildMetadataSection(input[cursor..section_end]);
...
fn isBuildMetadataSection(payload: []const u8) Error!bool {
    var cursor: usize = 0;
    ...
    const name = payload[cursor..name_end];
    return std.mem.eql(u8, name, "name") or std.mem.startsWith(u8, name, ".debug_");
}
...
fn fatal(init: std.process.Init, comptime format: []const u8, args: anytype) noreturn {
    ...
    stderr_writer.interface.print("strip-wasm-metadata: " ++ format ++ "\n", args) catch {};
    stderr_writer.interface.flush() catch {};
    std.process.exit(1);
}

test "removes name and DWARF custom sections" {
    const input =
        "\x00asm\x01\x00\x00\x00" ++
        "\x00\x05\x04name" ++
        "\x00\x0c\x0b.debug_info" ++
    ...
    const expected =
        ...
        "\x00\x0a\x09producers" ++
        "\x01\x01\x00";
    const output = try stripMetadataSections(std.testing.allocator, input);
    defer std.testing.allocator.free(output);
    try std.testing.expectEqualSlices(u8, expected, output);
}

test "rejects malformed modules" {
    try std.testing.expectError(error.InvalidHeader, stripMetadataSections(std.testing.allocator, "not wasm"));
    try std.testing.expectError(
        error.InvalidSection,
        stripMetadataSections(std.testing.allocator, "\x00asm\x01\x00\x00\x00\x00\x05\x04na"),
    );
}
