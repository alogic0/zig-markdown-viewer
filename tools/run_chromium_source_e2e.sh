...
if [ "$#" -ne 3 ]; then
    echo "usage: run_chromium_source_e2e.sh EXTENSION_DIR RENDERER_WASM SOURCE_FILE" >&2
    exit 2
fi
...
cp "${source_file}" "${work_dir}/sample.zig"
...
source_url="file://${work_dir}/sample.zig"
...
echo "Chromium source viewer E2E: local Zig file rendered with highlighting"
